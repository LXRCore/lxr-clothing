...
RegisterNetEvent('lxr-clothing:client:newPlayer', function()
...
RegisterNetEvent('lxr-clothing:client:openMenu', function(newPlayer, menuType)
...
RegisterNUICallback('closeMenu2', function()
    ...
    ClothingRoomTransition(BeforePosition, false)
    FreezeEntityPosition(playerPed, false)
    TriggerServerEvent('lxr-clothing:server:saveSkin', model, skins, clothes)
end)
...
RegisterNUICallback('saveOutfit', function(data)
    ...
    local model = GetEntityModel(playerPed)
    local clothes = json.encode(ClothesData)
    TriggerServerEvent('lxr-clothing:server:saveOutfit', data.outfitName, model, clothes)
    DisableCamera()
end)
