--[[
    ...
    🐺 LXR Clothing System — Server Main
    Server-side handlers for skin/outfit persistence
...
Framework.CreateCallback('lxr-clothing:server:isPlayerNew', function(source, cb)
    local src    = source
...
Framework.CreateCallback('lxr-clothing:server:getOutfits', function(source, cb)
...
AddEventHandler("lxr-clothing:server:removeOutfit", function(outfitName, outfitId)
    local src    = source
    local Player = Framework.GetPlayer(src)
    MySQL.query.await('DELETE FROM player_outfits WHERE citizenid = @citizenid AND outfitId = @outfitId', {
        ['@citizenid'] = Player.PlayerData.citizenid,
        ['@outfitId']  = outfitId
    })
    local result = MySQL.query.await('SELECT * FROM player_outfits WHERE citizenid = @citizenid', { ['@citizenid'] = Player.PlayerData.citizenid })
